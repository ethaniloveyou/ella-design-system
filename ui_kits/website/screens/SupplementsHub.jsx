...
export function SupplementsHub({ lang, setRoute }) {
  const t = useT(lang);
  const isEs = lang === 'es';
  ...
  return (
    <div data-screen-label="06 Supplements">
      <section className="ella-section">
        <div className="ella-container">
          <div className="ella-section-head" style={{ maxWidth: 760 }}>
            <div className="ella-eyebrow">{t('nav_supplements').toUpperCase()}</div>
            <h2>{isEs ? 'M\u00e1s importantes que un multivitam\u00ednico, ahora.' : 'More important than a multivitamin, at this stage.'}</h2>
          </div>

          <div className="ella-supp-grid">
            {items.map((sup, idx) => (
              <button
                key={sup.name}
                type="button"
              ...
              >
                <div style={{
                  ...
                  background: toneBg(sup.tone), color: toneFg(sup.tone),
                }}>
                  <IconDroplet width="24" height="24" />
                </div>
                <span className="ella-supp-tag" style={{ background: toneBg(sup.tone), color: tagFg(sup.tone) }}>{sup.tag}</span>
                <h4>{sup.name}</h4>
                <p>{sup.desc}</p>
                <span className="ella-pillar-cta" style={{ marginTop: 'auto', paddingTop: 6 }}>
                  {isEs ? 'Ver el porqu\u00e9 y la ciencia' : 'See the why & the science'} <IconArrow width="14" height="14" />
                </span>
              </button>
            ))}
          </div>
      ...
      {/* ---------- Detail modal ---------- */}
      {s && (
        <div className="ella-supp-modal-overlay" onClick={() => setOpen(null)} role="dialog" aria-modal="true" aria-label={s.name}>
          <div className="ella-supp-modal" onClick={(e) => e.stopPropagation()}>
            <button type="button" className="ella-supp-modal-close" aria-label={isEs ? 'Cerrar' : 'Close'} onClick={() => setOpen(null)}>\u00d7</button>
            <div className="ella-supp-modal-head">
              <div style={{
                width: 52, height: 52, borderRadius: 14,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: toneBg(s.tone), color: toneFg(s.tone), flex: '0 0 52px',
              }}>
                <IconDroplet width="26" height="26" />
              </div>
              <div>
                <span className="ella-supp-tag" style={{ background: toneBg(s.tone), color: tagFg(s.tone) }}>{s.tag}</span>
                <h3>{s.name}</h3>
              </div>
            </div>

            <h4 className="ella-supp-modal-sub">{isEs ? 'Por qu\u00e9 importa ahora' : 'Why it matters now'}</h4>
            <ul className="ella-supp-why">
              {s.why.map((w, i) => <li key={i}>{w}</li>)}
            </ul>

            <div className="ella-supp-dose">
              <span className="ella-supp-dose-lbl">{isEs ? 'C\u00d3MO TOMARLO' : 'HOW TO TAKE IT'}</span>
              <p>{s.dose}</p>
            </div>

            <h4 className="ella-supp-modal-sub">{isEs ? 'La investigaci\u00f3n' : 'The research'}</h4>
            <p className="ella-supp-research">{s.research}</p>
            {s.source && (
              <a className="ella-supp-source" href={s.source.href} target="_blank" rel="noopener noreferrer">
                {s.source.label}
                <IconArrow width="15" height="15" style={{ transform: 'rotate(-45deg)' }} />
              </a>
            )}

            <div className="ella-supp-modal-actions">
              <a className="ella-btn ella-btn-primary" href="#" onClick={(e) => e.preventDefault()}>
                {isEs ? 'Ver marca recomendada' : 'See recommended brand'}
                <IconArrow width="18" height="18" />
              </a>
            </div>
            <div className="ella-supp-modal-disc">
